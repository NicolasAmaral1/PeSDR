...
def test_removeprefix_is_safe_on_bare_keys() -> None:
    """If schema ever relaxes the secrets/ prefix requirement, factory still works."""
    cfg = LLMConfig.model_construct(
        provider="anthropic", model="m", api_key_ref="bare_key", temperature=None
    )
    with patch("ai_sdr.llm.factory.init_chat_model") as fake:
